...
class EnvService:
    ...
    async def cleanup_inactive_instances(self):
        ...
        for instance_id in instances_to_release:
            try:
                ...
                print(f"Error releasing inactive instance {instance_id}: {e}")
    ...
    def get_remote_env_cls(self, env_type: str):
        ...
        @ray.remote(num_cpus=1, memory=1024 * 1024 * 1024, max_restarts=-1, max_task_retries=-1)
        class RemoteEnv:
            """
            ...
            def __init__(self, task_id, instance_id, params):
                ...
                try:
                    module = importlib.import_module(
                        f"env_service.environments.{env_type}."
                        f"{env_type}_env",
                    )
                    envir_class = getattr(
                        module,
    ...
    async def release_instance(self, instance_id: str) -> bool:
        ...
        try:
            ...
            await asyncio.wait_for(env_actor.close.remote(), timeout=5.0)
        except Exception as e:
            print(f"Error closing instance {instance_id}: {e}")
        finally:
            # Always kill and cleanup
