...
def sanitize_data(data):
    if isinstance(data, dict):
        ...
        if math.isinf(data): return 999999.0 if data > 0 else -999999.0
    return data

# 2. 劫持 JSONResponse 的 render 方法
original_render = JSONResponse.render

def patched_render(self, content) -> bytes:
    # 在序列化前清洗数据
    clean_content = sanitize_data(content)
    return json.dumps(
        clean_content,
        ensure_ascii=False,
        allow_nan=True, # 即使清洗漏了，也允许生成（虽然前端可能还报错，但至少后端不崩溃）
        indent=None,
        separators=(",", ":"),
    ).encode("utf-8")

JSONResponse.render = patched_render
print("Successfully patched JSONResponse to handle Inf/NaN values.")
