...
def run_ppo(config) -> None:
    ...
    if not ray.is_initialized():
        ...
        # Fix for "Unable to register worker with raylet" error
        # 1. include_dashboard=False: saves ports and resources
        # 2. ignore_reinit_error=True: prevents crashes on re-init
        # 3. _temp_dir: uses user space to avoid /tmp issues
        ray.init(
            ...
            _temp_dir=os.path.expanduser("~/tmp/ray"),
...
class TaskRunner:
    ...
    This class is decorated with lsof .remote to ensure it is scheduled on a worker node and not the head node.
    """
    def run(self, config):
        """
