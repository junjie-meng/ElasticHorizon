...
from agentevolver.utils.max_steps_curriculum import (
    MaxStepsCurriculumConfig,
    MaxStepsCurriculumController,
)
...
class AgentEvolverRayPPOTrainer(RayPPOTrainer):
    ...
    def fit(self):
        ...
        # ------------------------------
        # MaxSteps ACL-V3 curriculum controller (optional)
        # ------------------------------
        curriculum_cfg = None
        self._max_steps_controller: MaxStepsCurriculumController | None = None
        if getattr(self.config, "curriculum", None) is not None:
            curriculum_cfg = MaxStepsCurriculumConfig(
                enable_max_steps_control=bool(self.config.curriculum.get("enable_max_steps_control", False)),
                update_mode = str(self.config.curriculum.get("update_mode", "acl")),
                min_max_steps=int(self.config.curriculum.get("min_max_steps", 1)),
                max_max_steps=int(self.config.curriculum.get("max_max_steps", 64)),
                headroom_steps=int(self.config.curriculum.get("headroom_steps", 2)),
                smoothing_alpha=float(self.config.curriculum.get("smoothing_alpha", 0.2)),
                success_sample_window=int(self.config.curriculum.get("success_sample_window", 100)),
                metrics_window=int(self.config.curriculum.get("metrics_window", 50)),
                min_success_samples_for_update_turns=int(
                    self.config.curriculum.get("min_success_samples_for_update_turns", 10)
                ),
                success_outcome_threshold=float(self.config.curriculum.get("success_outcome_threshold", 0.5)),
            )

        self.global_steps = 0

        if curriculum_cfg is not None :
            initial = int(self.config.actor_rollout_ref.rollout.multi_turn.max_steps)
            self._max_steps_controller = MaxStepsCurriculumController(
                cfg=curriculum_cfg,
                initial_current_max_steps=initial,
            )

        tracker.log(
            data={
                'curriculum/current_max_steps': float(initial),
                'curriculum/success_samples_count': 0,
            },
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        ...
                        if not self.async_rollout_mode:
                            ...
                            # ------------------------------
                            # rollout pre-hook:
                            #   - always record turns-related metrics (post-hook)
                            #   - ONLY override max_steps when curriculum.enable_max_steps_control is True
                            # ------------------------------
                            applied_max_steps = int(self.config.actor_rollout_ref.rollout.multi_turn.max_steps)
                            if (
                                self._max_steps_controller is not None
                                and bool(self._max_steps_controller.cfg.enable_max_steps_control)
                            ):
                                applied_max_steps = self._max_steps_controller.get_max_steps_override()
                                # override config so AgentFlow reads it during construction
                                self.config.actor_rollout_ref.rollout.multi_turn.max_steps = applied_max_steps
                            # log applied_max_steps regardless so dashboards can compare curriculum vs fixed
                            metrics["curriculum/applied_max_steps"] = applied_max_steps
                            ...
                            # ------------------------------
                            # rollout post-hook: compute batch curriculum metrics + update controller
                            # ------------------------------
                            if self._max_steps_controller is not None:
                                turns_list = []
                                success_flags = []
                                clipped_flags = []
                                successful_turns = []

                                threshold = float(self._max_steps_controller.cfg.success_outcome_threshold)


                                for traj in trajectories:
                                    steps = getattr(traj, "steps", None)
                                    # turns = int(len(steps)) if steps is not None else 0
                                    if isinstance(steps, list):
                                        turns = sum(1 for msg in steps if isinstance(msg, dict) and msg.get('role') == 'assistant')
                                        # turns = ( len(steps) - 1 ) / 2
                                    else:
                                        turns = 0
                                    ...
                                    try:
                                        if traj.reward is not None:
                                            outcome = float(traj.reward.outcome)
                                    except Exception:
                                        outcome = None

                                    success = bool(outcome is not None and outcome >= threshold)
                                    success_flags.append(success)
                                    if success:
                                        successful_turns.append(turns)

                                    # prefer explicit truncation reason if ever added; fallback to heuristic
                                    clipped = False
                                    try:
                                        # heuristic: hit max_steps and still not terminated
                                        clipped = (turns >= int(applied_max_steps)) and (not bool(getattr(traj, "is_terminated", False)))
                                    except Exception:
                                        clipped = False
                                    clipped_flags.append(clipped)

                                turns_arr = np.array(turns_list, dtype=np.float32) if turns_list else np.array([], dtype=np.float32)
                                success_rate = float(np.mean(success_flags)) if success_flags else 0.0
                                clip_ratio = float(np.mean(clipped_flags)) if clipped_flags else 0.0

                                batch_curr_metrics = {
                                    "curriculum/success_rate": success_rate,
                                    "curriculum/turns/clip_ratio": clip_ratio,
                                }
                                if turns_list:
                                    batch_curr_metrics.update(
                                        {
                                            "curriculum/turns/min": float(np.min(turns_arr)),
                                            "curriculum/turns/max": float(np.max(turns_arr)),
                                            "curriculum/turns/mean": float(np.mean(turns_arr)),
                                            "curriculum/turns/p50": float(np.percentile(turns_arr, 50)),
                                            "curriculum/turns/p90": float(np.percentile(turns_arr, 90)),
                                        }
                                    )
                                metrics.update(batch_curr_metrics)

                                ctrl_metrics = self._max_steps_controller.update(
                                ...
                                # structured log when current_max_steps changes by >=1
                                if self._max_steps_controller.should_log_step_change():
                                    logger.bind(
                                        global_step=int(self.global_steps),
                                        epoch=int(epoch),
                                        iter=int(i),
                                        old_max_steps=float(ctrl_metrics.get("curriculum/debug/old_current_max_steps", 0.0)),
                                        new_max_steps=float(ctrl_metrics.get("curriculum/current_max_steps", 0.0)),
                                        p90_success_turns=float(ctrl_metrics.get("curriculum/debug/p90_success_turns", -1.0)) if "curriculum/debug/p90_success_turns" in ctrl_metrics else None,
                                        target=float(ctrl_metrics.get("curriculum/debug/target_max_steps", -1.0)) if "curriculum/debug/target_max_steps" in ctrl_metrics else None,
                                        headroom_steps=int(self._max_steps_controller.cfg.headroom_steps),
                                        smoothing_alpha=float(self._max_steps_controller.cfg.smoothing_alpha),
                                        success_rate=float(success_rate),
                                        clip_ratio=float(clip_ratio),
                                        success_samples_count=int(self._max_steps_controller.success_samples_count()),
                                        reason=str(ctrl_metrics.get("curriculum/debug/reason", "")),
                                    ).info("[curriculum/max_steps] update")
